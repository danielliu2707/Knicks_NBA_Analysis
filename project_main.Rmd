...
Persona: PRESENT MYSELF AS A DATA ANALYST AS A KNICKS FAN WHO TRIES TO PROPOSE SOLUTIONS TO HELP THE KNICKS
SUCCEED AS AN NBA FRANCHISE. Therefore, explain every concept (like draft picks, salary etc...) to help
basically people who don't understand basketball understand my analysis.
...
# Nba player salaries: Requires web-scraping to extract 2021 and 2022 data.
...
# Web scraping to get 2021/2022 NBA Salary Data:

# NOTE: COULD CREATE A USER-DEFINED FUNCTION!!! WOULD REDUCE CODE SIGNIFICANTLY HERE.

page_22 <- read_html("https://www.spotrac.com/nba/cap/2022/")

table_22 <- page_22 %>% 
  html_table()

table_22 <- data.frame(table_22) %>% 
  as_tibble() %>% 
...
table_22$salary <- as.numeric(table_22$salary)

# Looks like it's now in the same format as salary so that we can bind the rows, adding on data salary data for 2022:
table_22
...
page_21 <- read_html("https://www.spotrac.com/nba/cap/2021/")

table_21 <- page_21 %>% 
  html_table()

table_21 <- data.frame(table_21) %>% 
  as_tibble() %>% 
  select(Team, Total.Cap) %>% 
  rename("salary" = "Total.Cap",
         "team" = "Team") %>% 
...
table_21$salary <- as.numeric(table_21$salary)
...
# Analysis 3: Impacts of poor drafting by the New York Knicks.
...
draft_picks %>% 
  ...
  summarise_at(vars(avg_net_rating),
...
Professional players are selected on average, 4.3 picks lower than college players, making it seem like no surprise that
college players would usually perform better than International players as International players seem to be selected lower.

However, the top performing international players (top 25%) perform better than that of the top performing college players.
These are usually higher picks (often in the top 14 selections known as the Lottery). Therefore, we can analyse whether
the Knicks are using their picks on International players who on average, if selected with higher picks (which the Knicks
have had since 2013 due to poor performances) will perform better on average than college players.
...
As seen in the graph, the Knicks are 2nd in terms of International players drafted, with 35.3% of our draft picks being International.
...
percent_drafted <- draft_picks %>% 
  ...
  arrange(desc(international_percent))
